...
Server.startServer = function(server) {
  ...
  for (var i = 0; i < scheduledTasks.length; i++) {
    crontab.scheduleJob(scheduledTasks[i].schedule, function(commands, randval) {
      // if the time was randomized then factor that in
      var randtime = 0;
      if (randval != undefined) {
        randtime = Math.random() * randval;
      }

      setTimeout(function(commands) {
        for( var j = 0; j < commands.length; j++) {
          setTimeout(function(command) {
            sendCommand(command);
          }.bind(undefined, commands[j]), commands[j].delay);
        }
      }.bind(undefined, commands), randtime);
    }.bind(undefined, scheduledTasks[i].commands, scheduledTasks[i].rand));
  }
};
